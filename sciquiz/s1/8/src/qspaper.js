	...
	for (i=0; i<totalq; i++) {
		...
		for(j=0;j<totaloptions;j++) {
			...
			if ( quizJSON.questions[rnq].a[rna].correct == true ) {
				...
				//s = s + '<div class="row"><div class="question"><span class="num"></span><span class="txt">' + 'જવાબ:  ' +  quizJSON.questions[rnq].a[rna].option + '</span></div>';
				//uck s = s + '<div class="row"><span class="txt">' + 'જવાબ:  ' +  quizJSON.questions[rnq].a[rna].option + '</div>';				
				//console.log( anssheet );
			}
		}
		//uck s = s + '</ul><br class="clear" /></div>';
		s = s + '</div>';
		//console.log( s );				
	}
	$('#qs').html(s);
	//uck $('.answers').html(anssheet);
