	...
	$.slickQuiz = function (element, options) {
		...
		plugin.method = {
				...
				for (i in questions) {
					if (questions.hasOwnProperty(i)) {
						...
						*/
						...
						questionHTML.append('<div class="qcontainer"><div class="questionnumber">' + count + '.</div>' + '<div class="questiontext"><h3>' + question.q + '</h3></div></div>');
						...
						for (i in answers) {
							if (answers.hasOwnProperty(i)) {
								...
								// uck add style="clear:left; 27-2-2014 wrap in if statement
								if (2 == i ) {
									var answerContent = $('<li style="clear:left;"></li>')
										.append(input)
										.append(optionLabel);
								} else {
			...
			checkAnswer: function (checkButton) {
				...
				// added by Ujamshi Khandla
				var uckQID = '';
				var tempQ = '';
				var tempTA = '';
				var tempSA = '';
				uckQID = '#' + questionLI.attr('id') + ' .questiontext h3';				
				//console.log(uckQID);
				//console.log( 'question: ' + $(uckQID).text() );
				tempQ = $(uckQID).text();
				//uckQuestions.push($(uckQID).text());
				//-uck
				...
				for (i in answers) {
					if (answers.hasOwnProperty(i)) {
						...
						if (answer.correct) {							
							//uck to check sup sub html ans trueAnswers.push($('<div />').html(answer.option).text());
							trueAnswers.push($('<div />').html(answer.option).html());
							// added by Ujamshi Khandla
							//console.log('true ans: ' + answer.option);
							tempTA = answer.option;
							//uckTrueAnswers.push(answer.option);
							//-uck
						}
					}
				}
				...
				answerInputs.each(function () {
					...
					// added by Ujamshi Khandla
					//console.log('selected ans: ' + inputValue);
					tempSA = inputValue;
					//uckSelectedAnswers.push(inputValue);
					//-uck
				});

				if (plugin.config.preventUnanswered && selectedAnswers.length === 0) {
					alert('કોઈ પણ એક વિકલ્પ પર ક્લિક કરવું ફરજિયાત છે.');
					return false;
				}
				// added by Ujamshi Khandla
				// Only one answer selected
				uckQuestions.push(tempQ);
				uckTrueAnswers.push(tempTA);
				uckSelectedAnswers.push(tempSA);
				//-uck
			...
			nextQuestion: function (nextButton) {
				...
				if (nextQuestion.length) {
					currentQuestion.fadeOut(300, function () {
						plugin.method.completeQuiz_uck();
			...
			completeQuiz: function () {
				var levels = [
					quizValues.info.level1,
					quizValues.info.level2,
					quizValues.info.level3,
					quizValues.info.level4,
					quizValues.info.level5
				],
			...
			// Hides all questions, displays the final score and some conclusive information
			completeQuiz_uck: function () {

				//php
				var uck_quizScore = $(_element + ' ' + _correct).length
				...
				// $('#trueans').html(score + ' / ' + questionCount);
				$('#trueans').show();
				$('#trueans').html(score);
			...
			calculateLevel: function (correctAnswers) {
				var percent = (correctAnswers / questionCount).toFixed(2),
					level = null;
				if (plugin.method.inRange(0, 0.349, percent)) {
					level = 4;
				} else if (plugin.method.inRange(0.35, 0.499, percent)) {
					level = 3;
				} else if (plugin.method.inRange(0.50, 0.649, percent)) {
					level = 2;
				} else if (plugin.method.inRange(0.65, 0.799, percent)) {
					level = 1;
				} else if (plugin.method.inRange(0.80, 1.000, percent)) {
					level = 0;
				}
				return level;
			},
			...
			// Calculates knowledge level based on number of correct answers
			calculateLevel_original: function (correctAnswers) {
				var percent = (correctAnswers / questionCount).toFixed(2),
					level = null;
...
// added by Ujamshi Khandla dt 14-2-2014
var uckQuestions = [];
var uckTrueAnswers = [];
var uckSelectedAnswers = [];
function drawQs() {
	// quizLevel or quizResults or quizResultsCopy

	var resultsheetHTML = '<div class="resultsheet"><ul>';
	resultsheetHTML = resultsheetHTML + '<li class="rsqshead">';
	resultsheetHTML = resultsheetHTML + '<span class="rsqs rsqsh">';
	resultsheetHTML = resultsheetHTML + 'પ્રશ્નો';
	resultsheetHTML = resultsheetHTML + '</span>';
	resultsheetHTML = resultsheetHTML + '<span class="rssa rsqsh">';
	resultsheetHTML = resultsheetHTML + 'તમારો જવાબ';
	resultsheetHTML = resultsheetHTML + '</span>';
	resultsheetHTML = resultsheetHTML + '<span class="rsta rsqsh">';
	resultsheetHTML = resultsheetHTML + 'સાચો જવાબ';
	resultsheetHTML = resultsheetHTML + '</span>';
	resultsheetHTML = resultsheetHTML + '</li>';

	var i = 0;
	var rsqclass = '';

	for (i=0;i<uckQuestions.length;i++) {
		if ( uckSelectedAnswers[i] === uckTrueAnswers[i] ) {
			rsqclass = 'rsq-correct';
		} else {
			rsqclass = 'rsq-incorrect';
		}
		resultsheetHTML = resultsheetHTML + '<li class="' + rsqclass + '">';
		resultsheetHTML = resultsheetHTML + '<span class="rsqs">';
		resultsheetHTML = resultsheetHTML + (i+1) + '. ' + uckQuestions[i];
		resultsheetHTML = resultsheetHTML + '</span>';
		resultsheetHTML = resultsheetHTML + '<span class="rssa">';
		resultsheetHTML = resultsheetHTML + uckSelectedAnswers[i];
		resultsheetHTML = resultsheetHTML + '</span>';
		resultsheetHTML = resultsheetHTML + '<span class="rsta">';
		resultsheetHTML = resultsheetHTML + uckTrueAnswers[i];
		resultsheetHTML = resultsheetHTML + '</span>';
		resultsheetHTML = resultsheetHTML + '</li>';
	}

	resultsheetHTML = resultsheetHTML + '</ul></div>';

	if ( true === showResultsheet ) {
		$( resultsheetHTML ).insertAfter( ".quizResultsCopy" );
	}
...
$(".startQuiz").click(function () {
	...
	setInterval(function () {
		if ($('.quizResults').css("display") == "block") {
			if (final_elapsed == "") {
				final_elapsed = get_elapsed_time_string(elapsed_seconds);
				$('.TimerText').html(final_elapsed);
				$('.TimerWrap').hide();
				$('.TimerTextWrap').show();
				$('.saveresult').show();
			}
		...
		$('.Timer').html(get_elapsed_time_string(elapsed_seconds));
	}, 1000);
})
...
function getcurrentdate() {
	...
	var d = new Date();
	var sd = d.getDate() + " " + month[d.getMonth()] + " " + d.getFullYear();
	return sd;
}
